...
I18N.register({
  'zh-TW': {
    ...
    'coord.start': '起始編號',
    'coord.start.0': '從 0 開始',
    'coord.start.1': '從 1 開始',
    'coord.position': '位置',
    ...
    'guide.color.desc': '從調色盤設定顏色與不透明度。縮小比例為 100% 時，為了避免相鄰的邊重疊，每個六角格只畫 3 條邊（因此不透明度會原樣呈現）。',
    'guide.width.desc': '線的寬度（px）。開啟發光效果時，發光範圍也會隨線寬等比例擴大。',
    'guide.style.desc': '可以選擇實線、虛線或點線。調低縮小比例後會畫出全部 6 條邊，虛線與點線的樣子也會跟著改變。',
    'guide.glow.desc': '讓線條發光，在深色背景上特別顯眼。下載的圖片也會套用。',
    'guide.shrink.desc': '六角格之間的距離（每格占用的範圍）不變，只縮小六角格本身畫出來的大小。低於 100% 時邊不再重疊，會畫出全部 6 條邊。想在 CCFOLIA 上讓六角格之間留出空隙時很方便。',
    'guide.coord': '座標顯示',
    'guide.format.desc': '可以選擇座標的顯示樣式：<strong>1-1</strong>（連字號分隔）、<strong>1, 1</strong>（逗號分隔）、<strong>0101</strong>（補零成 4 位數）、<strong>A1</strong>（欄用英文字母、列用數字）、<strong>流水號</strong>（不用座標，依序編號）。',
    ...
    'guide.posOffset.desc': '選擇座標文字要放在上、中、下哪個位置。選「上」或「下」時，可以用「距邊偏移」調整文字與邊的距離（px）。<strong>正值會往中心靠近</strong>，負值則會移到六角格外側。這個距離會隨縮小比例連動。'
  },
  ja: {
    'app.title': 'ヘクス作成ツール',
